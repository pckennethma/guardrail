...
def parse_cfd_line(line: str) -> Optional[Tuple[List[Tuple[str, str]], Tuple[str, str]]]:
...
def apply_cfds(df: pd.DataFrame, cfd_lines: List[str]) -> pd.DataFrame:
    ...
    for idx, cfd_line in enumerate(cfd_lines):
        cfd_result: Optional[Tuple[List[Tuple[str, str]], Tuple[str, str]]] = parse_cfd_line(cfd_line)
        ...
        df.loc[violation_mask, '_CTANE_ERROR'] = df.loc[violation_mask, '_CTANE_ERROR'].apply(lambda x: x + [cfd_id])
...
def main():
    ...
    if args.test:
        ...
        data: Dict[str, List[Union[str, int, float, None]]] = {
            'Occupation': ['Machine-op-inspct', 'Other-service', 'Prof-specialty', 'Other-service', '?'],
            'Workclass': ['Private', 'Private', 'Self-emp', 'State-gov', '?'],
            'Marital-status': ['Never-married', 'Married', 'Never-married', 'Married', 'Divorced'],
            'Income': ['LessThan50K', 'LessThan50K', 'GreaterThan50K', 'LessThan50K', 'LessThan50K'],
            'Relationship': ['Husband', 'Own-child', 'Not-in-family', 'Other-relative', 'Unmarried'],
            'Age': [18, 25, 40, 18, 30],
