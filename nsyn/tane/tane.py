...
logger = get_logger(name="nsyn.tane")
    ...
    with open(path, "r") as fin:
        for t, line in enumerate(fin):
            ...
            for i, s in enumerate(line.split(",")):
                hashes.setdefault(i, {}).setdefault(s, set([])).add(
                    t
                )  # [(i, s)] = len(hashes)
        return [
            PPattern.fix_desc(list(hashes[k].values())) for k in sorted(hashes.keys())
        ]
...
class PartitionsManager(object):
    ...
    def register_partition(self, X, X0, X1):
        ...
        """
        self.cache[len(X)][X] = PPattern.intersection(
            self.cache[len(X0)][X0], self.cache[len(X1)][X1]
        )
...
class rdict(dict):
    ...
    def recursive_search(self, key):
        return reduce(
            set.intersection,
            [self[tuple(key[:i] + key[i + 1 :])] for i in range(len(key))],
        )
...
class TANE(object):
        ...
        for X in L:
            for y in self.Cplus[X].intersection(X):
                ...
                if self.pmgr.check_fd(LHS, y):
                    ...
                    map(
                        self.Cplus[X].remove,
                        filter(lambda i: i not in X, self.Cplus[X]),
                    )
    ...
    def prune(self, L):
        ...
        for X in L:
            if not bool(self.Cplus[X]):
                clean_idx.add(X)
            if self.pmgr.is_superkey(
                X
            ):  # Is Superkey, since it's a stripped partition, then it's an empty set
                for y in filter(lambda x: x not in X, self.Cplus[X]):
                    if y in reduce(
                        set.intersection,
                        [
                            self.Cplus[tuple(sorted(X[:b] + X[b + 1 :] + (y,)))]
    ...
    def run(self, df: pd.DataFrame) -> DSLProg:
        """
        Procedure TANE in [1]
        """
        ...
        prog = DSLProg()
        rule_dict: Dict[int, tuple[DSLStmt, float]] = {}
        for LHS, y in self.rules:
            if len(LHS) >= 4:
                logger.warning(
                    f"Skipping rule with LHS {[columns[i] for i in LHS]} and RHS {columns[y]}"
                )
                continue
            stmt = DSLStmt.create(
                dependent=columns[y], determinants=[columns[i] for i in LHS]
            )
            stmt.force_fit(df)
            logger.info(stmt)
            if y not in rule_dict or rule_dict[y][1] < stmt.compute_loss(df):
                rule_dict[y] = (stmt, stmt.compute_loss(df))

        for y, (stmt, loss) in rule_dict.items():
            prog.add_stmt(stmt)
        return prog
...
if __name__ == "__main__":
    ...
    T = read_db(args.input)
    df = pd.read_csv(args.input)
    tane = TANE(T)
    t0 = time.time()
    prog = tane.run(df)
    logger.info("\t=> Execution Time: {} seconds".format(time.time() - t0))
    logger.info("\t=> {} Rules Found".format(len(tane.rules)))
    logger.info(prog)
