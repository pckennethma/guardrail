...
class TestSearch(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.prog = self.search.run()

    def test_error_detector(self) -> None:
        assert isinstance(self.prog, DSLProg)
        detector = ErrorDetector(data=self.test_data, program=self.prog)
        errors = detector.run()
