...
def main(
    ...
    log_folder = os.path.join(directory, model_folder, "sampler")
    ...
    if not disable_synthesizer:
        ...
        run_search(
            ...
            output_file=os.path.join(
                log_folder,
                f"nsyn_prog_sampler_use_aux_{enable_auxiliary_sampler}_idx_{exp_idx}.pkl",
            ),
            enable_auxiliary_sampler=True if enable_auxiliary_sampler == 1 else False,
            dag_save_path=os.path.join(log_folder, f"nsyn_dag_idx_{exp_idx}.pkl")
            if enable_auxiliary_sampler == 1
            else os.path.join(log_folder, f"nsyn_dag_identity_idx_{exp_idx}.pkl"),
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--enable_auxiliary_sampler",
        type=int,
        default=1,
        help="Whether to enable auxiliary sampler.",
    )
    parser.add_argument(
        "--exp_idx",
        type=int,
    ...
    if dataset_name is not None:
        main(
            ...
            enable_auxiliary_sampler=args.enable_auxiliary_sampler,
