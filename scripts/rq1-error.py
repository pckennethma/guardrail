...
def main(stat_paths: list[str], output_path: str):
    ...
    for index, statistics_and_dataset in enumerate(all_statistics):
        ...
        tn_data = (
            statistics["total_input_num"]
            - statistics["actual_data_error_num"]
            - fp_data
        )

        data_error_confusion_matrix_data = np.array(
            [[tp_data, fp_data], [fn_data, tn_data]]
        )
        data_error_normalized_confusion_matrix_data = (
            data_error_confusion_matrix_data.astype("float")
            / data_error_confusion_matrix_data.sum(axis=1)[:, np.newaxis]
        )

        logger.info(
            f"Confusion matrix for {dataset_alias_name}:\n{data_error_confusion_matrix_data}"
        )
        logger.info(
            f"Normalized confusion matrix for {dataset_alias_name}:\n{data_error_normalized_confusion_matrix_data}"
        )
        ...
        # tn_pred = (
        #     statistics["total_input_num"] - statistics["total_pred_error_num"] - fp_data
        # )

        # pred_error_confusion_matrix_data = np.array(
        #     [[tp_pred, fp_pred], [fn_pred, tn_pred]]
        # )
        # pred_error_normalized_confusion_matrix_data = (
        #     pred_error_confusion_matrix_data.astype("float")
        #     / pred_error_confusion_matrix_data.sum(axis=1)[:, np.newaxis]
        # )

        # ax_data_error = axs[0, index]  # Selects the subplot for 'Data Error' in the first row
        ax_data_error = axs[
            index
        ]  # Selects the subplot for 'Data Error' in the first row
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Visualize the statistics of relevance analysis."
    )
    parser.add_argument(
        "--output_path",
        "-o",
