...
class Q2Executor:
    ...
    def _model_group_by(
        ...
        grouped_df = df.groupby(
            f"{model_label} [predicted]", dropna=True
        )  # TODO: change to dropna=True when mode is coerce
        return grouped_df
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="ignore",
        choices=["ignore", "raise", "coerce", "rectify", "gt"],
        help="The error handling mode. Default: ignore.",
    )

    args = parser.parse_args()
    if args.mode == "gt":
        error_handling_mode = "gt"
    else:
        error_handling_mode = ERROR_HANDLING_FLAG
    ...
    if args.query_path is not None and os.path.isdir(args.query_path):
        ...
        query_list.sort()
        sql_files = [f for f in os.listdir(args.query_path) if f.endswith(".sql")]
        sql_files.sort()

        logger.info(f"Found {len(query_list)} queries.")
        json_log_name = "rq2_result.json"
        json_log_path = os.path.join(args.query_path, json_log_name)
        ...
        for i, query_path in enumerate(query_list):
            with open(query_path, "r") as f:
                logger.info(f"Executing query in {query_path} ...")
                sql_str = f.read()
                if error_handling_mode == "gt":
                    sql_str = sql_str.replace("noisy", "gt")
                else:
                    sql_str = sql_str.replace("noisy", "noisy_level_05")
                ...
                json_log[
                    sql_files[i] + "," + error_handling_mode
                ] = df.values.tolist()  # df.iloc[:, -1].to_list()
